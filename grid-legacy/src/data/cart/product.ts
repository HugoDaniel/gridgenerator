...
export enum ProductAt { TShirt = 100, Poster }
export enum PosterType { A1 = 'A1', A2 = 'A2', A3 = 'A3' }
export const PosterSizes = {
	A1: [59.4, 84.1],
	A2: [42, 59.4],
	A3: [29.7, 42]
};
export enum TShirtType { Man = 'Man', Woman = 'Woman', Unisex = 'Unisex' }
export enum TShirtSize { S = 'S', M = 'M', L = 'L', XL = 'XL' }
export enum TShirtColor { Black = '#25282B', White = '#FFFFFF', HeatherGray = '#ADBDBF' }
export class CartProduct {
	...
	public withArt(art: { svg: string, viewbox: [number, number, number, number] }) {
		this.artSVG = art.svg;
		this.artViewbox = art.viewbox;
		this.originalViewbox = art.viewbox.slice(0) as [number, number, number, number];
	}
	public zoom(z: number) {
		if (this.originalViewbox && this.artViewbox) {
			...
			const dw = (iz * maxW) * 2;
			const dh = (iz * maxH) * 2;
			const newvb = [-dw / 2, -dh / 2, maxW + dw, maxH + dh] as [number, number, number, number];
	...
	private centerRect(w: number, h: number): { w: number, h: number, x: number, y: number } {
	...
	public productType() {
		switch (this.at) {
			case ProductAt.Poster: return 'Poster';
			case ProductAt.TShirt: return 'T-Shirt';
		}
	}
	...
	public setPrice(p: IPrices) {
		...
		if (this.at === ProductAt.TShirt) {
			switch (this.tshirtType) {
				case TShirtType.Man: price = p.tshirtMan; break;
				case TShirtType.Woman: price = p.tshirtWoman; break;
				case TShirtType.Unisex: price = p.tshirtUnisex; break;
			}
		} else {
			switch (this.posterType) {
				case PosterType.A1: price = p.posterA1; break;
				case PosterType.A2: price = p.posterA2; break;
				case PosterType.A3: price = p.posterA3; break;
			}
		}
		this.price = price;
	}
