...
export const enum PatternHit { Inside = 'Inside', Outside = 'Outside', StartCircle = 'StartCircle', EndCircle = 'EndCircle' }
export class ClipPattern {
	...
	private isInside: (screenX: number, screenY: number) => boolean = (screenX, screenY) => {
		return screenX <= this.screenEndX && screenX >= this.screenStartX
		    && screenY <= this.screenEndY && screenY >= this.screenStartY;
	}
	/** Determines if a given screen point is inside this pattern starting circle */
	private isInStartCircle: (screenX: number, screenY: number) => boolean = (screenX, screenY) => {
		return Math.hypot(this.screenStartX - screenX, this.screenStartY - screenY) <= (2 * this.circleRadius);
	}
	/** Determines if a given screen point is inside this pattern starting circle */
	private isInEndCircle: (screenX: number, screenY: number) => boolean = (screenX, screenY) => {
