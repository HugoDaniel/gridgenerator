		...
		<section className={`ExportAnimation flex ${props.className || ''}`}>
			...
			<div className="info">
				...
				<div className="flex items-start justify-start flex-wrap gray">
					<a onClick={linkEvent(ExportEditorFormat.MP4, props.events.onFormatChange)} className={`flex items-center justify-center mt2 mr2 w3 h2 ba pa2 f7 link dim ttu dark-gray pointer ${props.data.format === ExportEditorFormat.MP4 ? 'bw2 b--blue black' : 'gray b--gray'}`}>MP4</a>
					<a onClick={linkEvent(ExportEditorFormat.GIF, props.events.onFormatChange)} className={`flex items-center justify-center mt2 mr2 w3 h2 ba pa2 f7 link dim ttu dark-gray pointer ${props.data.format === ExportEditorFormat.GIF ? 'bw2 b--blue black' : 'gray b--gray'}`}>GIF</a>
				</div>
			</div>
		</section>
	);
