...
export class State {
  ...
  constructor(maxSize: number = 256) {
    ...
    this._layers = new LayerMap(
      GridType.Square,
      shapeIds,
      shapeIds[0],
      shape.selectedFillSet
    );
  ...
  get selectedShape(): Shape {
    const shape = this._shapes.getShapeById(
      this._layers.getSelected().selectedShape
    );
    if (!shape) {
      throw new Error(
        "state.selectedShape -> No shape is selected in the current layer"
      );
    }
    return shape;
  }
  ...
  public renderSVG(dim: IGridDimension, w: number, h: number): string {
    ...
    return `
		<svg ${
      w && h ? `width="${w}px" height="${h}px"` : ""
    } viewBox="0 0 ${dim.width * res} ${dim.height *
      res}" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
