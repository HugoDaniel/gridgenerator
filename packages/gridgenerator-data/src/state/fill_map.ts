...
export class FillMap {
  ...
  public updateFromEditor(fillId: FillId): string {
    ...
    switch (this.fills.get(fillId)) {
      ...
      default:
        throw new Error(
          `updateFromEditor(${fillId}), Fill id not present or has an unknown type`
        );
    }
  }
  public updateEditorWith(fillId: FillId): FillMap {
    ...
    switch (this.fills.get(fillId)) {
      ...
      default:
        throw new Error(
          `updateEditorWith(${fillId}), Fill id not present or has an unknown type`
        );
    }
    return this;
  }
  ...
  public buildShapeSymbol(shape: Shape, shapeId: number): string[] {
    ...
    for (const [fillSetId, pathFills] of shape.entries()) {
      result.push(
        this.buildSVGSymbol(shape.resolution, shapeId, fillSetId, pathFills)
      );
    }
    return result;
  }
  ...
  public duplicateSingle(fillId: FillId, n: number, pool: number[]): void {
    ...
    for (let i = 0; i < n || i < pool.length; i++) {
      switch (t) {
        case FillType.Color:
          this.colors.registerColor(new RGBColor(c.r, c.g, c.b, c.a), pool[i]);
          break;
        default:
          continue;
      }
  ...
  public newFills(fillIds: FillId[], colors: RGBColor[]): FillMap {
    if (fillIds.length !== colors.length) {
      ...
      console.warn(
        "FillMap.newFills(): Different length in fillIds and colors"
      );
    }
