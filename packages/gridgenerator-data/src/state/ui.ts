...
import {
  UIShapeEditor,
  UIShapeEditorMode,
  UIShapeEditorReviver
} from "./ui/shape_editor";
...
export class UI {
  ...
  public static revive(o: UIReviver) {
    ...
    if (o.p) {
      result.patterns = new Map(
        o.p.map(
          ([lid, pat]) =>
            [lid, new ClipPattern(TilePattern.revive(pat))] as [
              LayerId,
              ClipPattern
            ]
        )
  ...
  public refreshMenus(
    _grid: Grid,
    _shapesMap: ShapeMap,
    svgs: Map<ShapeFillSetId, string>
  ) {
  ...
  private stateFromFeature(feature: FeaturesMenuId | string) {
    switch (feature) {
      case FeaturesMenuId.Export:
        return UIState.Export;
      case FeaturesMenuId.Save:
        return UIState.Save;
      case FeaturesMenuId.Publish:
        return UIState.Publish;
      case FeaturesMenuId.Product:
