...
export class LayerMap {
  ...
  public toJSON(): LayerMapReviver {
    return {
      l: [...this._layers.entries()].map(
        e => [e[0], e[1].toJSON(g => g.toJSON())] as [number, LayerReviver]
      ),
  ...
  public static revive(o: LayerMapReviver) {
    ...
    result._layers = new Map(
      o.l.map(
        e => [e[0], Layer.revive(e[1], Grid.revive)] as [LayerId, Layer<Grid>]
      )
    );
    result.list = o.lst;
    result.selected = o.s;
  ...
  public getSelected(): Grid {
    ...
    if (!l) {
      throw new Error(
        `layer_map: Cannot get the selected layer ${this.selected}: Not found`
      );
    }
    return l.data;
  }
