...
export class UIFillEditor {
  ...
  public static revive(o: UIFillEditorReviver) {
    ...
    result.paths = new Map(
      o.p.map(e => [e[0], UIFillPath.revive(e[1])] as [FillId, UIFillPath])
    );
    return result;
  }
  public buildPaths(paths: string[], fillIds: number[], fills: string[]) {
    ...
    for (let i = 0; i < paths.length; i++) {
      this.paths.set(fillIds[i], new UIFillPath(paths[i], fills[i]));
    }
  }
  public selectedFillString(): string {
    ...
    if (!uifill) {
      throw new Error(
        `selectedFillString(): selected fill id ${this.selected} not found`
      );
    }
    return uifill.fill;
  }
  public updateSelected(fillValue: string): UIFillEditor {
    ...
    if (!uifill) {
      throw new Error(
        `updateSelected(${fillValue}): selected fill id ${
          this.selected
        } not found`
      );
    }
