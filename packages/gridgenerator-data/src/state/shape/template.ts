...
export class TemplateElement {
  ...
  private intersectLineEllipse(l: number[], e: number[]): Vector2D[] {
    ...
    const qb =
      ...
      2 * h * h * b * cosa * sina;
    const qc =
      v * v * b * b * sina * sina + h * h * b * b * cosa * cosa - h * h * v * v;
    const roots = solveQuadraticEquation(qa, qb, qc).filter(
      num => num && !isNaN(num)
    ) as number[];
  ...
  private intersectVLineEllipse(l: number[], e: number[]): Vector2D[] {
    ...
    const qc =
      x * x * (v * v * cosa * cosa + h * h * sina * sina) - h * h * v * v;
    const roots = solveQuadraticEquation(qa, qb, qc).filter(
      num => num && !isNaN(num)
    ) as number[];
    const intersections = roots.map(_cr => {
  ...
  public static line(
    ...
    return new TemplateElement([x1, y1, x2, y2], type);
  }
  public static ellipse(
    ...
    return new TemplateElement([cx, cy, rx, ry, alpha], ElementType.Ellipse);
  }
}
...
export interface TemplateReviver {
  base: number;
  res: number;
  rot: number;
  elements: TemplateElementReviver[];
}
export class Template {
  ...
  public static revive(o: TemplateReviver) {
    return new Template(
      o.base,
      o.elements.map(TemplateElement.revive),
      o.res,
      o.rot
    );
  }
    ...
    for (let i = 0; i < num; i++) {
      ...
      switch (elem.type) {
        ...
        case ElementType.Ellipse:
          ...
          result += `M${cx - rx} ${cy} a${rx} ${ry} 0 1 0 ${rx *
            2} 0 a${rx} ${ry} 0 1 0 -${rx * 2} 0`;
      }
    }
    return result;
  }
  ...
  public getReachable(pt: Vector2D): VectorSet {
    ...
    for (const i of this.intersections) {
      if (
        !pt.isEqual(i.point) &&
        (elementsIndices.has(i.elementA) || elementsIndices.has(i.elementB))
      ) {
        result.add(i.point);
      }
    }
  ...
  public getReachableFrom(
    ...
    for (const i of this.intersections) {
      if (
        !withoutPts.has(i.point) &&
        !edge1.isEqual(i.point) &&
        !edge2.isEqual(i.point) &&
        (elementsIndices.has(i.elementA) || elementsIndices.has(i.elementB))
      ) {
        result.add(i.point);
      }
...
export const squareRoundTrisTemplate: () => Template = () =>
  new Template(4, [
    // square:
...
export const squareDiagTemplate: () => Template = () =>
  new Template(4, [
    // square:
    TemplateElement.line(0, 0, 512, 0), // 0: th
    TemplateElement.line(512, 0, 512, 512), // 1: rv
...
export const squareBasicTemplate: () => Template = () =>
  new Template(4, [
    // square:
    TemplateElement.line(0, 0, 512, 0), // 0: th
    TemplateElement.line(512, 0, 512, 512), // 1: rv
