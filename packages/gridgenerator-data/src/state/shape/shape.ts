...
export class Shape {
  ...
  public static revive(o: ShapeReviver) {
    ...
    const fills = new Map(
      o.pf.map(f => [f[0], new Map(f[1])] as [number, Map<string, number>])
    );
  ...
  get selectedPathFillId(): number {
    ...
    if (!fillId) {
      throw new Error(
        `FillId not found for the selected path ${this._selectedPath}`
      );
    }
    return fillId;
  }
  ...
  public maxFillIds(): number {
    return (
      this.pathFills.size *
      Math.max(this._editor.fillIds.length, this.getSelectedFills().size)
    );
  }
  /**
    ...
    for (const [d, fillId] of selected.entries()) {
      if (!pathFillIds.has(fillId)) {
        ...
        if (!newD) {
          ...
          console.warn(
            "Should not happen, new d string not found for fillId",
            fillId
          );
          result.deleted.set(d, fillId);
        } else if (newD !== oldD) {
  ...
  public updateShape(
    p: Path,
    duplicateIds: Map<number, number[]>,
    cmds: IUpdatedShapeCmds
  ) {
  ...
  public updateEditorFills(): Path {
    ...
    if (!fills) {
      throw new Error(
        `Cannot discard new Shape fills: No selected fill found in shape ${
          this.selected
        }`
      );
    }
  ...
  public selectFill(fid: number): Shape {
    if (this.pathFills.has(fid)) {
      ...
      throw new Error(
        `Cannot select fill because fillId '${fid}' is not present in this brush path fills`
      );
    }
  }
  public removeSelectedFill(): Shape {
    ...
    if (this.pathFills.size === 1) {
      ...
      throw new Error(
        `Cannot delete because there is no selected fill '${this.selected}' `
      );
    }
