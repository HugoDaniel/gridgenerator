...
export class ShapeMap {
  ...
  constructor(
    ...
    shapeId: number = 1,
    fillsForDefaultShape = [
      ColorDefaults.YELLOW,
      ColorDefaults.BLUE,
      ColorDefaults.GREEN
    ]
  ) {
    this.templates = new Map();
    this.templates.set(GridType.Square, [
      squareBasicTemplate(),
      squareDiagTemplate(),
      squareRoundTrisTemplate()
    ]);
    ...
    this.shapes.set(
      shapeId,
      Shape.FullSquare(fillsForDefaultShape, defaultTemplate)
    );
    this.created = shapeId;
  }
  public toJSON(): ShapeMapReviver {
    return {
      s: [...this.shapes.entries()].map(
        sm => [sm[0], sm[1].toJSON()] as [number, ShapeReviver]
      ),
      c: this.created,
      t: [...this.templates.entries()].map(
        ts =>
          [ts[0], ts[1].map(temp => temp.toJSON()) as TemplateReviver[]] as [
            number,
            TemplateReviver[]
          ]
      ),
  ...
  public static revive(o: ShapeMapReviver) {
    const templates = new Map(
      o.t.map(
        smt => [smt[0], smt[1].map(Template.revive)] as [number, Template[]]
      )
    );
    const shapes = new Map(
      o.s.map(sms => [sms[0], Shape.revive(sms[1])] as [ShapeId, Shape])
    );
  ...
  public getShapeChanges(shapeId: number): IUpdatedShapeCmds {
    ...
    if (s) {
      ...
      throw new Error(
        `ShapeId ${shapeId} not found to analyze the updates needed`
      );
    }
  }
  public getMaxNeededDups(shapeId: number): number {
    ...
    if (s) {
      ...
      throw new Error(
        `ShapeId ${shapeId} not found to get the max fill ids that are needed`
      );
    }
  }
  public getNeededDups(shapeId: number, cmds: IUpdatedShapeCmds): IDupsNeeded {
    ...
    if (s) {
      ...
      throw new Error(
        `ShapeId ${shapeId} not found to get the duplicate fill ids that are needed`
      );
    }
  }
  public saveUpdatedShape(
    type: GridType,
    shapeId: ShapeId,
    shapeFillId: ShapeFillSetId,
    dups: Map<number, number[]>,
    changes: IUpdatedShapeCmds
  ): Shape {
    // this.created = shapeId;
