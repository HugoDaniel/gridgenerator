...
class PathAction {
	...
	constructor(actionType: ActionType, args: Array<number | boolean>, element: TemplateElement | null) {
	...
	public static Reverse(pt: Vector2D, prevPt: Vector2D, a: PathAction): (PathAction | undefined) {
		if (!a.element) {
	...
	public static Arc(ellipse: TemplateElement, origin: Vector2D, dest: Vector2D): PathAction[] {
	...
	private static elementsFromOriginToDest(t: Template, origin: Vector2D, dest: Vector2D): Set<TemplateElement> {
	...
	public static Select(t: Template, origin: Vector2D, dest: Vector2D): PathAction[] {
...
class PathShape {
	...
	public modShape(t: Template, a: PathAction[], reaches: VectorSet, curEdge: Vector2D | null, otherEdge: Vector2D | null, pts: Vector2D[]) {
	...
	public static templateString(t: Template) {
		const template = new Template(
			...
			t.resolution);
		let shape = new PathShapeInstance(template, [], new VectorSet(), null, null, []);
...
class PathShapeInstance {
	...
	constructor(t: Template, a: PathAction[], reaches: VectorSet, curEdge: Vector2D | null, otherEdge: Vector2D | null, pts: Vector2D[]) {
		this.template = t;
		this.actions = a;
	...
	public isClosed(): boolean {
		...
		return (
			(this.selectedPoints[0].isEqual(lastPt))
			|| lastAction.isClosed()
		);
	}
	...
	public calcActivePts(): VectorSet {
		...
		if (lastAction.actionType === ActionType.Arc) {
			...
			if (longPoints) {
				return (
					pts.filter( (pt) => !ellipse.ellipseIsBetween( pt1, pt2, pt))
				);
			} else {
				return (
					pts.filter( (pt) => ellipse.ellipseIsBetween(pt1, pt2, pt) )
...
export class Path {
	...
	public discardCurrent(): void {
		...
		this.shapes[this.shapes.length - 1] =
			new PathShape(this.shapes[this.shapes.length - 1].template);
		this.ambiguities = [];
	}
