...
export class Grid {
  ...
  constructor(
    ...
    selectedShapeFillId: ShapeFillSetId,
    canvas: GridCanvas = new VectorMap()
  ) {
  ...
  public renderSVGUse(
    ...
    for (const [gridElem, [x, y]] of this._canvas.entries()) {
      ...
      result.push(`
			<use xlink:href="#${gridElem.shapeId}-${
        gridElem.fillSetId
      }" x="${posx}" y="${posy}" ${
        useRes ? `width="${res + 2}" height="${res + 2}"` : ''
      } transform="scale(${1}) rotate(${gridElem.rotation * 360}, ${posx +
        res / 2}, ${posy + res / 2})" />
			`);
    }
    return result;
  ...
  public rotateSelected(): Rotation {
    ...
    if (rotIndex === undefined) {
      throw new Error(
        `Cannot rotate the shape: selected shape not found ${
          this._selectedShape
        }`
      );
    }
    ...
    const newElem = new GridElement(
      this._curElement.shapeId,
      this._curElement.fillSetId,
      rot
    );
