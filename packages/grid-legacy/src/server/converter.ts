...
async function convertMp4(work, w, h, dest): Promise<{ mp4: string }> {
  ...
  while (fatState.version !== fatState.maxVersion) {
    ...
    frames.push(`
		<svg width="${w}" height="${h}" viewBox="${viewbox.join(
      ' '
...
function convertImage(destFile, w, h, svgBody, viewbox) {
  ...
  const svg = `
		<svg width="${w}" height="${h}" viewBox="${svgviewbox.join(
    ' '
  )}" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
			${svgBody}
		</svg>`;
  return new Promise((resolve, reject) => {
    converter.convert(svg).then(
      (result) => {
        writePNG(destFile, result).then(
          (file) => {
            resolve(result);
          },
          (error) => {
...
let server = app.listen(port, () => {
  ...
  app.post('/convert/png', function(req, res) {
    ...
    const userId = req.body.userId;
    const svg = `
			<svg width="${width}" height="${height}" viewBox="${req.body.viewbox.join(
      ' '
    )}" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
			${req.body.svg}
			</svg>`;
    converter.convert(svg).then((result) => {
      ...
      writePNG(`${dir}/${pngFileName}`, result).then(
        (file) => {
  ...
  app.post('/convert/publish', async function(req, res) {
    ...
    convertImage(
      ...
      (imgData) => {
        ...
        h = Math.ceil(h / 1.6);
        try {
          convertMp4(req.body, w, h, publishDir).then(
            (mp4Response) => {
              res.setHeader('Content-Type', 'application/json');
