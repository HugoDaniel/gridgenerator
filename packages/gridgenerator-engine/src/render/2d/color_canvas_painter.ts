import { State, Wheel, WheelMode } from "gridgenerator-data";
...
export class ColorCanvasPainter {
  public static INIT(ctx: ColorPickerCanvasCtx, _state: Readonly<State>) {
    ...
    if (state.mode === WheelMode.WHEEL_HERING_MODE) {
      ...
      ColorCanvasPainter._heringCircle(
        ctx,
        ctx.width,
        state.heringColors,
        state.hering,
      ...
      ColorCanvasPainter._unitCircle(
        ctx,
        ctx.width,
        state.selectedSlice,
        state.slices,
        state.ringRatio1,
        state.ringRatio2,
        state.brightnessColors,
        state.brightnessOffset,
        state.mode
      );
      ...
      ColorCanvasPainter._unitCircle(
        ...
        state.ringRatio1,
        state.ringRatio2,
        state.saturationColors,
        state.saturationOffset,
        state.mode
      );
    }
    // Paint the circle slices
    ColorCanvasPainter._circleSliceN(
      ctx.ctx,
      cx,
      cy,
      state.slices,
  ...
  private static _renderDragInfo(
    context: ColorPickerCanvasCtx,
    wheel: Wheel,
    cx: number,
    cy: number
  ) {
    ...
    const r = 3;
    const distOut =
      wheel.ringRatio1 * size -
      ((wheel.ringRatio1 - wheel.ringRatio2) * size) / 2;
    ...
    const arrowLen = 1.25;
    const downXarrow1 =
      cx + Math.cos(a + sliceAngle / arrowLen) * (distOut + r);
    const downYarrow1 =
      cy + Math.sin(a + sliceAngle / arrowLen) * (distOut + r);
    const downXarrow2 =
      cx + Math.cos(a + sliceAngle / arrowLen) * (distOut - r);
    const downYarrow2 =
      cy + Math.sin(a + sliceAngle / arrowLen) * (distOut - r);
  ...
  private static _renderPrecision(
    context: ColorPickerCanvasCtx,
    wheel,
    cx,
    cy,
    ...
    for (let i = 0; i <= totalRulerLines; i++) {
      ...
      if (a + apos > a2 + sliceAngle) {
        ...
        a =
          ...
          a2 +
          offset -
          (totalRulerLines - i + 1) * lineSpacingAngle;
      } else {
        a += apos;
      }
  ...
  private static _renderBackground(
    canvas: ColorPickerCanvasCtx,
    wheel: Wheel,
    cx: number,
    cy: number
  ): void {
  ...
  public static startMovement(
    _state: Readonly<State>,
    mov: Movement,
    rect: DOMRect | ClientRect
  ): IColorCanvasMovementDetail | number | null {
  ...
  private static _renderTris(ctx, slices, posR, cx, cy, selected, color) {
    ...
    ctx.lineTo(
      cx + Math.cos(a + sliceAngle / w) * posR,
      cy + Math.sin(a + sliceAngle / w) * posR
    );
    ctx.lineTo(
      cx + Math.cos(a - sliceAngle / w) * posR,
      cy + Math.sin(a - sliceAngle / w) * posR
  ...
  private static _renderSelectedColor(
    canvas: ColorPickerCanvasCtx,
    wheel: Wheel
  ) {
  ...
  public static wheelMoving(ctx: ColorPickerCanvasCtx, state: Readonly<State>) {
    ...
    if (state.fills.colors.editor.mode === WheelMode.WHEEL_HERING_MODE) {
      ...
      state.fills.colors.editor.mode === WheelMode.WHEEL_BRIGHTNESS_MODE
    ) {
      const c = state.fills.colors.editor.brightnessColors;
      ColorCanvasPainter.unitMove(
        ctx,
        state,
        c,
        state.fills.colors.editor.brightnessOffset
      );
    } else if (
      state.fills.colors.editor.mode === WheelMode.WHEEL_SATURATION_MODE
    ) {
      ...
      ColorCanvasPainter.unitMove(
        ...
        state,
        c,
        state.fills.colors.editor.saturationOffset
      );
    }
  }

  public static unitMove(
    canvas: ColorPickerCanvasCtx,
    ...
    ColorCanvasPainter._renderPrecision(
      canvas,
      state.fills.colors.editor,
      cx,
      cy,
      2 * Math.PI - (offset * Math.PI) / 180
    );
  }
  ...
  private static _createCacheCtx(
    size: number,
    canvas: ColorPickerCanvasCtx,
    mode: WheelMode,
    value
  ...
  private static _unitCircle(
    realCtx: ColorPickerCanvasCtx,
    size,
    pos,
    slices,
    ratio1,
