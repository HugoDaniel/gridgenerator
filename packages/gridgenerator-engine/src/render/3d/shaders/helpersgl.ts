...
export class HelpersGL {
  ...
  private static putTextureInGPU(
    gl: WebGLRenderingContext,
    toGPU: (gl: WebGLRenderingContext) => void,
    doMipMap: boolean = false
  ): WebGLTexture {
  ...
  public static textureArray(
    gl: WebGLRenderingContext,
    _texture: ArrayBufferView,
    width: number,
    height: number,
    doMipMap: boolean = false
  ): WebGLTexture {
    const toGPU = (_gl: WebGLRenderingContext) => {
      return _gl.texImage2D(
        gl.TEXTURE_2D,
        0,
        gl.RGBA,
        width,
