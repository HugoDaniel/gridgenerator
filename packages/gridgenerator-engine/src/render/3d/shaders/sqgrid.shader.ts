...
export class SqGridShader {
  ...
  private setCursorColor(): number[] {
    const color = this._state.fills.getFillObj(
      this._state.currentLayer.cursorColor
    );
    if (color) {
  ...
  private drawTextures(drawBuffer: ShaderBuffer) {
    ...
    if (this.textureIdsChanged) {
      ...
      if (this.textureIdsBuffer) {
        this.textureIdsBuffer = HelpersGL.bufferFloat32(
          this.gl,
          this.textureIds,
          this.textureIds.length / 2,
          2,
          false,
          this.textureIdsBuffer
        );
      }
    ...
    if (this.textureRotBuffer) {
      this.textureRotBuffer = HelpersGL.bufferFloat32(
        this.gl,
        this.textureRot,
        this.textureIds.length,
        1,
        false,
        this.textureRotBuffer
      );
    } else {
      this.textureRotBuffer = HelpersGL.bufferFloat32(
        this.gl,
    ...
    this.gl.vertexAttribPointer(
      this.shader.loc.attrib("aFaceRot"),
      this.textureRotBuffer.itemSize,
      this.gl.FLOAT,
      false,
      0,
      0
    ...
    for (let u = 0; u < this.clipspace.uv.length; u++) {
      ...
      for (let sq = 0; sq < squares; sq++) {
        ...
        const verts = 4;
        for (let v = 0; v < verts; v++) {
          this.textureCoords.set(
            coords,
            sq * verts * coords.length + v * coords.length
          );
        }
      }
      if (this.textureCoordsBuffer) {
        this.textureCoordsBuffer = HelpersGL.bufferFloat32(
          this.gl,
          this.textureCoords,
          this.textureCoords.length / 4,
          4,
          true,
      ...
      this.gl.vertexAttribPointer(
        this.shader.loc.attrib("aFaceTextureCoords"),
      ...
      this.gl.bindTexture(
        this.gl.TEXTURE_2D,
        this.shapeTextures.getGLTexture(u)
      );
  ...
  public init() {
    ...
    this.draw = () => {
      ...
      this.shader.loc.set1Uniform(
        this.gl,
        "iUnitSize",
        this._state.viewport.unitSize
      );
      ...
      this.gl.vertexAttribPointer(
        this.shader.loc.attrib("aVertexIdd"),
        this.buffers[1].itemSize,
        this.gl.FLOAT,
        false,
        0,
        0
      ...
      this.gl.vertexAttribPointer(
        this.shader.loc.attrib("aFaceIndex"),
  ...
  public initBuffers(gl: WebGLRenderingContext): ShaderBuffer[] {
    ...
    const bufVertexPositionData = HelpersGL.buffer(
      gl,
      aVertexPositionData,
      aVertexPositionData.length / 3,
      3
    );
    const bufVertexIdData = HelpersGL.buffer(
      gl,
      aVertexIdData,
      aVertexIdData.length,
      1
    );
    const bufFaceIndexData = HelpersGL.buffer(
      gl,
      aFaceIndexData,
      aFaceIndexData.length,
      1
    );
    const bufFaceVertexIdData = HelpersGL.buffer(
      gl,
      aFaceVertexIdData,
      aFaceVertexIdData.length,
      1
    );
    const indices = HelpersGL.buffer(
      gl,
      aFaceVertexIdData,
      aFaceVertexIdData.length,
      1,
    ...
    const lineIndices = HelpersGL.buffer(
      gl,
      lines,
      lines.length,
      1,
