...
export class ShaderAnimation {
  ...
  private run(f: FrameRequestCallback) {
    if (!this.finished) {
      ...
      this.runningTime = this.reverseTime(
        this.direction,
        this.runningTime,
        dur,
        revDur
  ...
  public draw(
    gl: WebGLRenderingContext,
    loc: Locations,
    shapeIdsBuffer: ShaderBuffer,
    shapeDelaysBuffer: ShaderBuffer,
    shapeDelaysRevBuffer: ShaderBuffer
  ) {
    this.setUniforms(gl, loc);
    gl.bindBuffer(gl.ARRAY_BUFFER, shapeIdsBuffer.buffer);
    gl.vertexAttribPointer(
      loc.attrib("aShapeIndex"),
      shapeIdsBuffer.itemSize,
      gl.FLOAT,
      false,
      0,
      0
    ...
    gl.vertexAttribPointer(
      loc.attrib("aShapeDelay"),
      shapeDelaysBuffer.itemSize,
      gl.FLOAT,
      false,
      0,
      0
    ...
    gl.vertexAttribPointer(
      loc.attrib("aShapeRevDelay"),
      shapeDelaysRevBuffer.itemSize,
      gl.FLOAT,
      false,
      0,
      0
