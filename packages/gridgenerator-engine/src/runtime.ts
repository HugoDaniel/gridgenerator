...
export class Runtime {
  ...
  public static setColorPickerCtx(
    rt: Runtime,
  ...
  public static resetClipSpace(
    rt: Runtime,
    state: Readonly<State>,
    dontTexturize?: boolean
  ): Promise<Runtime> {
    const error =
      "Cannot reset clip space because runtime has no TextureManager defined";
    const r: Promise<Runtime> = new Promise((resolve, reject) => {
      if (!dontTexturize) {
        rt.updateAllTextures(state).then(ta => {
          if (rt.textures) {
            rt.clipSpace.fromGrid(
              state.viewport,
              state.currentLayer,
              rt.textures
            );
  ...
  public static texturize(
    ...
    if (!rt.webglCtx) {
      ...
      console.trace();
      throw new Error(
        `Cannot texturize: No WebGL context available in the runtime`
      );
    } else if (!rt.textureCanvas) {
      // tslint:disable-next-line:no-console
      console.trace();
      throw new Error(
        `Cannot texturize: No texture canvas context available in the runtime`
      );
    } else if (!rt.textureImg) {
      // tslint:disable-next-line:no-console
      console.trace();
      throw new Error(
        `Cannot texturize: No texture image available in the runtime`
      );
    }
  ...
  public updateAllTextures(state: Readonly<State>): Promise<TextureAtlas[]> {
    ...
    if (this.textures) {
      ...
      this.textures = new TextureManager(
        this.device.dpr / 100,
        size,
        this.webglCtx.maxNumTextures,
        this.webglCtx.maxTextureSize / 4
      );
    }
