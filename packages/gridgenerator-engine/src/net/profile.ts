import { MeanderProfile } from "gridgenerator-data";
...
export class NetProfile {
  ...
  constructor(
    hostname: string,
    graphql: (queryStr: string, token?: Token) => Promise<any>,
    getData: (url: string, token?: Token) => Promise<any>
  ) {
  ...
  public async getProfile(t: Token) {
    return this.graphql(
      `{ curProfile { id, name, badges, about, createdAt } }`,
      t
    );
  }
