...
export class NetLogin {
  ...
  public async verifyEmail(searchLink: string) {
    return this.getData(this.hostname + "/auth/verify" + searchLink).then(
      response => response.json()
    );
  }
  public async recover(uname: string) {
    const d = { j: uname };
    return this.postData(this.hostname + "/auth/email/recover", d).then(
      response => response.json()
    );
  }
  public async resetPassword(newPass: string, searchLink: string) {
    ...
    return this.postData(
      this.hostname + "/auth/email/recover" + searchLink,
      d
  ...
  public async emailRegister(uname: string, p: string) {
    ...
    return this.postData(this.hostname + "/auth/email/create", d).then(
      response => response.json(),
  ...
  public async emailLogin(uname: string, p: string) {
    ...
    return this.postData(this.hostname + "/auth/email/login", d).then(
      response => response.json()
    );
  }
}
