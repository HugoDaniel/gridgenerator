import { FillId, UIFillEditor, UIFillEditorMode } from "gridgenerator-data";
...
import { RecentColors } from "./color_picker/recent";
import { ISystemPickerBtnProps } from "./color_picker/system";
...
function needsVertExpansion(
  isInPortrait: boolean,
  size: RuntimeMediaSize
): boolean {
  return size === RuntimeMediaSize.Normal && isInPortrait;
}
function needsHorizExpansion(
  isInPortrait: boolean,
  size: RuntimeMediaSize
): boolean {
...
export const ColorPicker = (props: IColorPickerProps) => {
  ...
  const systemPickerProps: ISystemPickerBtnProps = {
    className: `${
      runtime.device.hasSystemColorPicker ? "dn" : "absolute w3 h3 br-100"
    }`,
  ...
  return (
    <div
      className={`ColorPicker ${className ||
        ""} flex justify-center items-start editormw editor-shadow h-100-ns pr5-ns`}
      style={style || {}}
    >
      <div className={"flex flex-column justify-center items-center"}>
        ...
        {fillEditor.editorMode !== UIFillEditorMode.Code ? (
          <div className="center mt4-ns">
            <ColorShapes {...colorShapesProps} />
          </div>
        ) : (
          <div />
        )}
        {fillPaths[0].length > 1 ? <FiguresMenu {...figMenuProps} /> : <div />}
        <RecentColors
