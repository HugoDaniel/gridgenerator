...
import { ExportAt, PlayerState, UIExportEditor } from "gridgenerator-data";
...
import { ExportDone } from "./export/done";
import { ExportImage } from "./export/image";
import { ExportPayment } from "./export/payment";
...
function selectProductComponent(props: IExportProps) {
  switch (props.data.at) {
    case ExportAt.Image:
      return <ExportImage data={props.data} events={props.events} />;
    case ExportAt.Video:
      return (
        <ExportAnimation
          data={props.data}
  ...
  return (
    ...
    >
      <section className="w-100 flex flex-column items-center justify-center">
        <h2 className="">Export project</h2>
        <div className="flex items-center justify-center gray">
          <a
            className={`pv2 ph4 ${
              props.data.at === ExportAt.Image
          ...
          >
            {" "}
            <img className="mr2 w1 h1" src={export_image} /> Image{" "}
          </a>
          <a
            className={`pv2 ph4 ${
              props.data.at === ExportAt.Video
                ? "z-1 br bl bt b--gray bg-white black bold"
                : "bg-transparent gray"
            } flex f6 link dim ttu pointer`}
          ...
          >
            {" "}
            <img className="mr2 w1 h1" src={export_animation} /> Animation{" "}
          </a>
        </div>
        <div
  ...
  return (
    ...
    >
      <section className="w-100 flex flex-column items-center justify-center">
        ...
        <p className="">
          {props.data.at === ExportAt.Preparing
            ? "Please wait (takes a couple of minutes)"
            : "Please wait"}
        </p>
        <div className="mt4 flex items-center justify-center">
          <Button
