...
export const ShapePoint = (props: IShapePointProps) => {
  if (props.pointAttribs.isOtherEdge) {
    ...
    return (
      <g
        ...
        style={{
          transform: `translate(${props.pointAttribs.x - 10}px, ${props
            .pointAttribs.y - 10}px)`,
          cursor: "pointer",
          width: "1rem",
          height: "1rem",
          background: "red"
  ...
  if (props.pointAttribs.isCurrentEdge) {
    ...
    cx += `orange-fill ${
      props.pointAttribs.isClickable ? "dark-gray-stroke pointer" : "no-stroke"
    } `;
  ...
  const _action =
    props.pointAttribs.isClickable && props.onAction
      ? linkEvent(props.pointAttribs, props.onAction)
      : null;
