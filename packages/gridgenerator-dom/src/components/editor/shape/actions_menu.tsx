...
function renderShapes(
  ...
  const result: any[] = [
    <p
      className={`sans-serif ttu mh1 ${
        shapes.length > 2 ? "ml2 w4 f7-ns f8 mv0" : "f6 tc w-100"
      }`}
...
export const ShapeActionsMenu = (props: IShapeActionsMenuProps) => {
  const title =
    props.shapes.length > 1
      ? props.title || "Or Go Back To: "
      : "Make your shape: connect the dots.";
  // console.log('ACTIONS MENU WITH', props.shapes.length);
  return (
    <nav
      className={`ShapeActionsMenu ${props.className ||
        ""} flex justify-start items-center w-100 bg-near-white overflow-hidden`}
      style={props.style || {}}
      $HasKeyedChildren
    >
      {renderShapes(
