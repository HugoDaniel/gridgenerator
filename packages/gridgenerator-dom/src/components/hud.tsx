...
import { FillId, ShapeId, ToolsMenuId, UI, UIState } from "gridgenerator-data";
import { RuntimeMediaSize } from "gridgenerator-engine";
...
function currentToolMenu(
  id: ToolsMenuId,
  fillsProps: IFillsMenuProps,
  deleteProps: IDeleteMenuProps,
  zoomProps: IZoomMenuProps
) {
  switch (id) {
...
export const HUD = (props: IHUDProps) => {
  ...
  const shapesMenuProps: IShapesMenuProps = {
    ...
    isOtherEditorVisible:
      props.ui.at === UIState.FillEditor ||
  ...
  const fillsMenuProps: IFillsMenuProps = {
    ...
    isOtherEditorVisible:
      props.ui.at === UIState.ShapeEditor ||
      props.ui.at === UIState.Export ||
      props.ui.at === UIState.Publish ||
      props.ui.at === UIState.PublishPreview ||
      props.ui.at === UIState.Product
  ...
  const featuresMenuProps: IFeaturesMenuProps = {
    className: `center h2 ma0 pa0 flex items-center justify-center transition-o ${
      props.ui.at === UIState.Project ? "o-100" : "o-0"
    }`,
  ...
  return (
    <section className={`HUD ${props.className || ""}`}>
      <h1
        className={`mb0 lh-title f6 w-100 tc sans-serif ttu black transition-transform ${
          props.ui.at === UIState.Project ? "" : "translate-y--3"
        }`}
        $HasVNodeChildren
      >
