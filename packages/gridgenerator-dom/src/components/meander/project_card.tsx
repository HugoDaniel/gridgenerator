...
import { StoredProject } from "gridgenerator-data";
...
export const ProjectCard = (props: IProjectCard) => {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="110" height="60" shape-rendering="crispEdges" viewBox="0 0 ${vbw} ${vbh}">${
    props.project.svg
  }</svg>`;
  ...
  return (
    <section className="ProjectCard tc pa3 w4">
      <a
        onClick={linkEvent(props.project.id, props.onView)}
        className="pointer link dim f6 gray"
      >
