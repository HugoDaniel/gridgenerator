...
export const Button = (props: IButtonProps) => (
  <button
    {...noPropagation}
    className={`Button avenir b--black-10 pa2 link f7 br1 transition-o ba ${
      props.disabled
        ? "bg-mid-gray light-gray o-0"
        : `bg-${props.bg || "blue"} pointer ${
            props.color ? props.color : "near-white"
          } dim o-100`
    } ttu ${props.className || ""}`}
    onClick={
      props.disabled
        ? null
        : props.id && props.onAction
        ? linkEvent(props.id, props.onAction)
        : props.onAction
    }
...
export const AmmountBtn = (props: IAmmountBtnProps) => {
  return (
    <div className="AmmountBtn flex items-center justify-center">
      ...
      <div className="bg-white ammount ttu pa2 f7 b--black-10 bt bb">
        {props.value}
      </div>
      <button
...
export const TextButton = (props: ITextButtonProps) => {
  ...
  const tachyonsCommon = `f8 ttu absolute b--none w${width ||
    3} h2 bg-transparent dark-gray`;
  ...
  const cx = `TextButton ${className || ""} bb b--dark-gray w${width ||
    3} w-expand-over`;
  return (
    <div className={cx}>
      <button
      ...
      <div className={`w${width || 3}-expand h2 bg-white bb b--dark-gray`} />
    </div>
  );
};
