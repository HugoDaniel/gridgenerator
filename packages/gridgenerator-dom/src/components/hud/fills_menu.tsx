...
import { Menu, ShapeFillSetId } from "gridgenerator-data";
...
function buildBtns(
  props: IFillsMenuProps,
  selectedCx: string,
  notSelectedCx: string
) {
  const result: any[] = [];
    ...
    result.push(
      ...
      >
        <div
          ...
          style={{
            transform: `rotate(${2 * Math.PI * (entry.rotation || 0)}rad)`
          }}
...
export const FillsMenu = (props: IFillsMenuProps) => {
  ...
  return (
    <nav
      className={`FillsMenu ${props.className ||
