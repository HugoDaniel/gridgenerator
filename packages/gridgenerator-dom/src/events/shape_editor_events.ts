...
export class ShapeEditorEvents implements IEventHandler {
  ...
  constructor(rt: Runtime, s: FatState, refresher: Refresher) {
    ...
    this.onTouchEnd = e => {
      ...
      if (!rect) {
        throw new Error(
          "Unable to handle mousedown in shape: no shape client rect"
        );
      }
      if (this.runtime.movement) {
        ...
        const templateRes = this.state.current.shapes.editor.template
          .resolution;
        // fetch the nearest point
        const pt = this.state.current.nearestActivePt(
          xparam * templateRes,
          yparam * templateRes
        );
        if (pt) {
    ...
    this.onPointAction = (e: IShapePointAttribs) => {
      ...
      if (e.isOtherEdge) {
        ...
        const fillId: FillId[] = this.state.current.newFillIds(
          this.runtime.rnd,
          1
        );
