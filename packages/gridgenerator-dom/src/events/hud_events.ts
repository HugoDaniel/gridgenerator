...
export class HUDEvents implements IEventHandler {
  ...
  constructor(
    ...
    s: FatState,
    refresher: Refresher,
    openScene: (onDone: () => void) => void,
    closeScene: (onDone: () => void) => void,
    redrawScene: (onDone: () => void) => void
  ) {
    this.runtime = rt;
    this.state = s;
    ...
    this.onNewFill = () => {
      ...
      const shapeFillSetId = this.state.current.newShapeFillSetId(
        this.runtime.rnd
      );
      ...
      if (this.runtime.colorPickerCtx) {
        ColorCanvasPainter.INIT(
          this.runtime.colorPickerCtx,
          this.state.current
        );
      }
    };
    ...
    this.onSelectTool = (toolsId: ToolsMenuId, e: Event) => {
      ...
      if (toolsId === ToolsMenuId.Undo) {
        ...
        this.runtime.getInitialState().then(
          initialState => {
            ...
            Runtime.resetClipSpace(this.runtime, this.state.current, true).then(
              ...
              error =>
                console.warn(
                  `Cannot reset runtime/clipspace to perform Undo action: ${error}`
                )
            );
          },
          // tslint:disable-next-line:no-console
          error =>
            console.warn(
              `Cannot get initial state to perform Undo action: ${error}`
            )
        );
      } else {
    ...
    this.onGridPattern = e => {
      ...
      if (this.state.current.ui.toolsSubmenus.isGridPatternOn) {
        ...
        if (!pattern) {
          ...
          this.state.hudNewPattern(
            v.squareLayerX() + centerX,
            v.squareLayerY() + centerY
          );
        }
      ...
      if (this.runtime.textures) {
        this.runtime.clipSpace.fromGrid(
          ...
          this.state.current.currentLayer.pattern !== null
        ); // <- hide/show original grid
        this.redrawScene();
      }
      this.refresher.refreshAll(this.runtime, this.state);
    };
  ...
  private async shape2Texture(update: boolean = false) {
    ...
    if (update) {
      ...
      for (const [fillSetId, m] of shape.entries()) {
        await this.runtime.updateTexture(
          shapeId,
          fillSetId,
          this.state.current.fills.buildSVG(shape.resolution, m, size, size)
        );
      }
  ...
  private patternify(layerX: number, layerY: number, show: boolean) {
    ...
    if (pattern) {
      if (
        this.state.current.ui.at === UIState.PatternAdjustEnd &&
        (pattern.gridPattern.endX === layerX &&
          pattern.gridPattern.endY === layerY)
      ) {
        return;
      } else if (
        pattern.gridPattern.startX === layerX &&
        pattern.gridPattern.startY === layerY
      ) {
        return;
      }
    }
    ...
    if (this.state.current.currentLayer.pattern && this.runtime.textures) {
      this.runtime.clipSpace.fromGrid(
        ...
        this.runtime.textures,
        show
      ); // <- show pattern grid (if true), or original grid (if false)
      this.redrawScene();
    }
    this.refresher.refreshStateAndDOM(this.state);
  }
