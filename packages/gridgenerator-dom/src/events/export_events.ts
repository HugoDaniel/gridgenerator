                          ...
                          this.net.export
                            ...
                            .then(
                              ...
                              fail => {
                                ...
                                console.log(
                                  "ERROR CONTACTING SERVER AFTER PAYMENT",
                                  fail
                                );
                                // TODO: render error
                              }
    ...
    this.onExportInit = () => {
      ...
      this.projects.getHash().then(hash => {
        ...
        if (!this.runtime.token) {
          ...
          this.net.export
            .postCanExport(this.runtime.token, hash)
            .then(response => {
    ...
    this.onExport = e => {
      ...
      if (exportEditor.at === ExportAt.Image) {
        if (exportEditor.format === ExportEditorFormat.PNG) {
            ...
            if (this.runtime.token) {
              this.net.export
                .postExportPNG(
                  this.runtime.token,
                  hash,
                  exportEditor.calcres(),
                  exportEditor.patternSize
                )
                .then(
                  exported => {
          ...
          // RENDER SVG
          const svg = this.state.current.renderSVG(
            exportEditor.dim,
            res.width,
            res.height
          );
        ...
        if (exportEditor.format === ExportEditorFormat.MP4) {
          this.projects.getHash().then(hash => {
            if (this.runtime.token) {
              this.net.export
                .postExportMP4(this.runtime.token, hash, exportEditor.calcres())
                .then(
                  exported => {
                    this.doneFile(exported.file);
                  },
          ...
          this.projects.getHash().then(hash => {
            if (this.runtime.token) {
              this.net.export
                .postExportGIF(this.runtime.token, hash, exportEditor.calcres())
                .then(
                  exported => {
                    this.doneFile(exported.file);
                  },
