...
export class MeanderEvents {
  ...
  constructor(
    ...
    refresher: Refresher,
    storeCurProj: () => void,
    restorePrevProj: (id: number) => Promise<void>,
    refreshProjs: (projs: StoredProject[], closeCurrent?: boolean) => void,
    getProj: (id: number) => StoredProject | undefined,
    reviveProj: (id: number) => Promise<Project>,
    reviveNetProj: (proj: any) => Promise<Project>
  ) {
    this.runtime = rt;
    this.meander = m;
    this.net = net;
    ...
    this.initFromToken = (tokenStr: string) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.refresher.refreshMeanderOnly(this.meander);
        this.updateDOM();
        this.net.profile.getProfile(token).then(
          profile => {
            const p = profile.data.curProfile;
            if (p && p.id) {
              this.meander.profile.buildProfile(
                p.id,
                p.name,
                p.about,
                p.createdAt,
              ...
              this.meander.profile.errorLoading(
                "Profile with no ID. Please get in contact with us."
              );
              this.updateDOM();
              resolve();
              return;
    ...
    this.onRouteLoginError = () => {
      ...
      if (query.length !== 2 || query[0] !== "?msg") {
        ...
        this.meander.login.error = `Login failed: ${decodeURIComponent(
          query[1]
        )}`;
      }
      this.updateDOM();
    ...
    this.onRouteViewProject = (projNum: string) => {
      ...
      if (!p) {
        ...
        this.net.profile
          .getProject(projId, this.runtime.token || undefined)
          .then(
            resp => {
              const data = resp.data;
              if (data.workById) {
                this.reviveNetProj(data.workById).then(
                  proj => {
                    ...
                    if (!stored) {
                      ...
                      console.log(
                        "ERROR, could not convert project to stored",
                        proj
                      );
                      return;
                    }
    ...
    this.onProfileUpdate = (e: Event) => {
      ...
      this.updateDOM();
      this.net.profile
        .updateProfile(this.runtime.token, this.meander.profile)
        .then(
          response => {
            if (
              !response.data ||
              !response.data.setProfile ||
              !response.data.setProfile.profile
            ) {
              this.meander.profile.errorLoading(
                "Invalid response from server when updating profile, please get in contact with us."
              );
              this.updateDOM();
              return;
            }
    ...
    this.onProfileProjects = () => {
      if (this.runtime.token) {
        this.meander.profile.startLoading();
        this.net.profile.getProfileProjects(this.runtime.token).then(
          response => {
    ...
    this.verifyEmail = (searchLink: string) => {
      this.net.login.verifyEmail(searchLink).then(
        res => {
    ...
    this.loginEmail = (e?: Event) => {
      ...
      this.getLoginData().then(
        loginData => {
          ...
          this.updateDOM();
          this.net.login.emailLogin(loginData.u, loginData.p).then(
            res => {
              if (!res.result) {
                // wrong credentials
    ...
    this.recover = (e: Event) => {
      ...
      this.getInputValue("login-u").then(
        username => {
          this.meander.login.isLoading = true;
          this.updateDOM();
          this.net.login.recover(username).then(
            answer => {
              if (answer.result) {
    ...
    this.resetPassword = (e: Event) => {
      ...
      this.getInputValue("recover-p").then(
        newPass => {
          ...
          this.updateDOM();
          this.net.login
            .resetPassword(newPass, window.location.search || "")
            .then(
              answer => {
                this.meander.login = new MeanderLogin();
                if (answer.result === false) {
                  this.meander.login.error = `Sorry ${
                    answer.reason
                  }. Please try to recover the password again`;
                  this.meander.login.showRecover = true;
                  this.updateDOM();
                } else {
                  this.meander.login.success =
                    "Password updated. Try to login with it.";
                }
                this.meander.course = MeanderCourse.Login;
                this.gotoLogin();
                this.updateDOM();
    ...
    this.registerEmail = (e?: Event) => {
      ...
      this.getLoginData().then(
        loginData => {
          this.meander.login.isLoading = true;
          this.updateDOM();
          this.net.login.emailRegister(loginData.u, loginData.p).then(
            (answer: ILoginResponse) => {
              this.meander.login.isLoading = false;
              this.meander.login.error = null;
              if (answer.result) {
                this.meander.login.success = answer.reason;
  ...
  private updateProjectView(proj: Project, p: StoredProject) {
    if (proj.fatState && proj.svg && proj.svgViewBox) {
      ...
      ps.canvasHeight = Math.min(
        (proj.svgViewBox[1] / proj.svgViewBox[0]) * ps.canvasWidth,
        this.runtime.device.height - 256
      );
  ...
  private async getLoginData(): Promise<ILoginData> {
    ...
    return this.getInputValue("login-u").then(
      username =>
        this.getInputValue("login-p").then(
          pass => ({ u: username, p: pass, error: null }),
          handleError
        ),
      handleError
    );
  ...
  private getProfileForm(): IProfileForm {
    const newNameElem = document.getElementById(
      "profile-name"
    ...
    const newBioElem = document.getElementById(
      "profile-bio"
  ...
  private getBillingInfo(): any {
    ...
    const vatidElem = document.getElementById(
      "vatid"
    ) as HTMLInputElement | null;
    const emailElem = document.getElementById(
      "email"
    ) as HTMLInputElement | null;
    const companyElem = document.getElementById(
      "company"
    ) as HTMLInputElement | null;
    const websiteElem = document.getElementById(
      "website"
    ) as HTMLInputElement | null;
    const phoneElem = document.getElementById(
      "phone"
    ) as HTMLInputElement | null;
    const regionElem = document.getElementById(
      "region"
    ) as HTMLInputElement | null;
    ...
    const streetElem = document.getElementById(
      "street"
    ) as HTMLInputElement | null;
