...
export class ColorPickerEvents implements IEventHandler {
  ...
  constructor(rt: Runtime, s: FatState, refresher: Refresher) {
    ...
    this.onMouseDown = e => {
      if (
        this.state.current.ui.fillEditor.editorMode === UIFillEditorMode.Color
      ) {
    ...
    this.onStartMovement = (x: number, y: number) => {
      ...
      if (!this.runtime.rects.colorPicker) {
        throw new Error(
          "Cannot start color picker movement event: No client rect for the color picker in runtime"
        );
      }
      ...
      if (detail !== null) {
        if (typeof detail === "number") {
          ...
          if (!this.runtime.colorPickerCtx) {
            throw new Error(
              "Cannot select color, runtime context is not present"
            );
          }
    ...
    this.onStopMovement = () => {
      ...
      if (!this.runtime.colorPickerCtx) {
        throw new Error(
          "Cannot stop color movement, runtime context is not present"
        );
      }
    ...
    this.onMovement = (_x: number, _y: number) => {
      ...
      if (typeof detail !== "number") {
        ...
        if (!rect) {
          throw new Error(
            "Cannot move color picker: no rectangle is defined in runtime"
          );
        }
        ...
        if (
          (detail as IColorCanvasMovementDetail).in ===
          WheelMode.WHEEL_HERING_MODE
        ) {
          ...
          ColorCanvasPainter.wheelMoving(
            this.runtime.colorPickerCtx,
            this.state.current
          );
        } else if (
          (detail as IColorCanvasMovementDetail).in ===
            WheelMode.WHEEL_BRIGHTNESS_MODE ||
          (detail as IColorCanvasMovementDetail).in ===
            WheelMode.WHEEL_SATURATION_MODE
        ) {
          if (!this.runtime.colorPickerCtx) {
            throw new Error("Canvas Ctx not present when moving color wheel");
          }
          ...
          if (!detailStart) {
            return new Error(
              'No "startAngle" in movement detail in color picker'
            );
          }
          ...
          if (!detailInit) {
            return new Error(
              'No "initValue" in movement detail in color picker'
            );
          }
    ...
    this.onCode = () => {
      if (
        this.state.current.ui.fillEditor.editorMode === UIFillEditorMode.Code
      ) {
