...
import { Net, Runtime, WebGLContext } from "gridgenerator-engine";
import {
  addMouse,
  addTouch,
  IEventHandler,
...
export class Events {
  ...
  // should updates:
  public shouldUpdateHUD: (
    lastProps: IHUDProps,
    nextProps: IHUDProps
  ) => boolean;
  public shouldUpdateScene: (
    lastProps: ISceneProps,
    nextProps: ISceneProps
  ) => boolean;
  public shouldUpdateMeander: (
    lastProps: IMeanderProps,
    nextProps: IMeanderProps
  ) => boolean;
  public shouldUpdateEditor: (
    lastProps: IEditorProps,
    nextProps: IEditorProps
  ) => boolean;
  public shouldUpdatePattern: (
    lastProps: IPatternProps,
    nextProps: IPatternProps
  ) => boolean;
  ...
  constructor(
    ...
    this.onMouseDown = (e: MouseEvent) => {
      if (
        ...
        this.state.current.ui.at === UIState.Publish ||
        this.state.current.ui.at === UIState.Export
      ) {
        return;
      }
      // e.preventDefault();
    ...
    this.projectEvents = new ProjectEvents(
      rt,
      s,
      this.net,
      this.projects,
      refresher,
      this.sceneEvents.reset
    );
    this.publishEvents = new PublishEvents(
      rt,
      s,
      this.net,
      refresher,
    ...
    this.productEvents = new ProductEvents(
      rt,
      s,
      this.cart,
      this.net,
    ...
    this.onboardingEvents = new OnboardingEvents(
      rt,
      s,
      refresher,
      this.onboarding
    ...
    this.hudEvents = new HUDEvents(
      ...
      s,
      refresher,
      this.sceneEvents.openCols,
      this.sceneEvents.closeCols,
      this.sceneEvents.onRedraw
    );
    ...
    this.meanderEvents = new MeanderEvents(
      rt,
      m,
      this.net,
      refresher,
  ...
  private getEventHandler(): IEventHandler {
    ...
    if (
      ...
      isAt === UIState.Publish ||
      this.state.current.ui.at === UIState.PublishPreview
    ) {
      ...
      isAt === UIState.PatternAdjustEnd ||
      isAt === UIState.PatternAdjustStart
    ) {
