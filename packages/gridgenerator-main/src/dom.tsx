...
import {
  ...
  removeMouse,
  removeTouch
} from "gridgenerator-dom";
import { Editor, IEditorProps } from "gridgenerator-dom";
import { HUD, IHUDProps } from "gridgenerator-dom";
import { ISceneProps, Scene } from "gridgenerator-dom";
import { Events } from "gridgenerator-dom";
import { IMeanderProps, MeanderFull } from "gridgenerator-dom";
import { Runtime, RuntimeMediaSize } from "gridgenerator-engine";
import { Pattern } from "gridgenerator-dom";
...
function configEditorBtn(
  props: IGridGeneratorDOMProps,
  ...
  switch (props.state.ui.at) {
    ...
    case UIState.ShapeEditor:
      if (props.state.ui.shapeEditor.shapesD.length === 0) {
        ...
        props.state.ui.shapeEditor.editorMode === UIShapeEditorMode.Fill
      ) {
        editorProps.actionLabel = props.state.ui.fillEditor.primaryActionTitle;
...
export class GridGeneratorDOM extends Component<IGridGeneratorDOMProps, any> {
  ...
  public render() {
    ...
    const hres =
      runtime.device.mediaSize === RuntimeMediaSize.Normal
    ...
    let editorProps: IEditorProps = {
      ...
      onPublishSuccess: () =>
        events.hudEvents.onExitFeatures(events.meanderEvents.gotoProjects),
    ...
    const meanderProps: IMeanderProps = {
      action: this.props.action,
      className: `${
        inProj ? "h3 children-o-0" : "h-100 bg-meander children-o-100"
      } w-100 f1 ttu fixed z-4 bottom-0 children-opacity`,
      ...
      <div style={{ cursor }}>
        <Scene
          ...
          onComponentShouldUpdate={
            this.props.events.shouldUpdateScene as (
              lastProps: any,
              nextProps: any
            ) => boolean
          }
        ...
        {pattern && !canCloseEditor ? (
          <Pattern
            ...
            onComponentShouldUpdate={
              this.props.events.shouldUpdatePattern as (
                lastProps: any,
                nextProps: any
              ) => boolean
            }
        ...
        <HUD
          {...hudProps}
          onComponentShouldUpdate={
            this.props.events.shouldUpdateHUD as (
              lastProps: any,
              nextProps: any
            ) => boolean
          }
        ...
        <Editor
          {...editorProps}
          onComponentShouldUpdate={
            this.props.events.shouldUpdateEditor as (
              lastProps: any,
              nextProps: any
            ) => boolean
          }
        ...
        <MeanderFull
          {...meanderProps}
          onComponentShouldUpdate={
            this.props.events.shouldUpdateMeander as (
              lastProps: any,
              nextProps: any
            ) => boolean
          }
