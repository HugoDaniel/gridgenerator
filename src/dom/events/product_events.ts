...
export class ProductEvents implements IEventHandler {
	...
	constructor(rt: Runtime, state: FatState, cart: Cart, net: Net, refresher: Refresher, proj: ProjectMap) {
		this.runtime = rt;
		this.state = state;
	...
	private onPosterMove(deltaX: number, deltaY: number, d: IProductMovementDetail) {
		if (deltaX !== this.cart.product.posterDeltaX
	...
	private onTShirtMove(deltaX: number, deltaY: number, d: IProductMovementDetail) {
		if (deltaX !== this.cart.product.tshirtDeltaX
	...
	private onMove(x: number, y: number) {
		...
		if (this.cart.product.at === ProductAt.TShirt) {
			this.onTShirtMove(deltaX, deltaY, this.runtime.movement.detail as IProductMovementDetail);
		} else {
			this.onPosterMove(deltaX, deltaY, this.runtime.movement.detail as IProductMovementDetail);
		}
	}
	private onUp() {
