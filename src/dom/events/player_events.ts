import { FatActionSets, FatState, PlayerState, Project, ProjectAction, ProjectLicense, State } from '../../data';
...
export class PlayerEvents {
	...
	public onRemix: () => void;
	public exit: () => void;
	constructor(rt: Runtime, p: PlayerState, refresher: Refresher, exit: () => void) {
		this.runtime = rt;
		this.state = p;
		...
		this.onPlayerCanvasInit = (ctx) => {
			...
			const svg = PlayerCanvasPainter.SVGHEAD(this.state.thumbnailSvg, ctx.width, ctx.height, this.state.thumbnailSvgViewBox);
		...
		this.onRemix = () => {
			...
			const newProj = new Project(State.revive(initial.toJSON()), ProjectAction.Fork, this.state.proj.license as ProjectLicense, fs);
	...
	private paint(s: Readonly<State>) {
		...
		this.state.currentViewBox = this.state.thumbnailSvgViewBox;
		const finalSVG = PlayerCanvasPainter.SVGHEAD(svg, this.state.canvasWidth, this.state.canvasHeight, this.state.currentViewBox);
