...
export class SceneEvents implements IEventHandler {
	...
	constructor(rt: Runtime, s: FatState, refresher: Refresher) {
		...
		this.onWebGLInit = (ctx: WebGLContext) => {
			Runtime.setWebGLCtx(this.runtime, ctx, this._state.current.viewport.maxSize);
			return Runtime.resetClipSpace(this.runtime, s.current).then((_rt: Runtime) => {
		...
		this.onTouchEnd = (e: TouchEvent) => {
			...
			if (this.curAction !== SceneAction.Move
				&& this.curAction !== SceneAction.Zoom && this.t1Start) {
	...
	private paintAt(x: number, y: number, layerX: number, layerY: number, shapeId: number, shapeFillId: number, rot: number) {
		...
		const sqIndex = ClipSpace.gridIndex(x, y, this._state.current.viewport, this.runtime.width);
		if (!this.runtime.textures) {
			throw new Error('Cannot paint: not runtime textures present');
		}
