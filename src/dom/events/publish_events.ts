import { FatState, Project, ProjectMap, State, UIPublishEditor } from '../../data';
...
export class PublishEvents implements IEventHandler {
	...
	constructor(rt: Runtime, s: FatState, net: Net, refresher: Refresher, proj: ProjectMap) {
		...
		this.onLicenseCCSA = (e: Event) => {
			...
			switch (this.state.current.ui.publishEditor.license) {
				case 'CC0':
				case 'BY': license = 'BY_SA'; break;
				case 'BY_SA': license = 'BY'; break;
				case 'BY_NC': license = 'BY_NC_SA'; break;
				case 'BY_ND': license = 'BY_SA'; break;
				case 'BY_NC_ND': license = 'BY_NC_SA'; break;
				case 'BY_NC_SA': license = 'BY_NC'; break;
			}
		...
		this.onLicenseCCNC = (e: Event) => {
			...
			switch (this.state.current.ui.publishEditor.license) {
				case 'CC0':
				case 'BY': license = 'BY_NC'; break;
				case 'BY_SA': license = 'BY_NC_SA'; break;
				case 'BY_NC': license = 'BY'; break;
				case 'BY_ND': license = 'BY_NC_ND'; break;
				case 'BY_NC_ND': license = 'BY_ND'; break;
				case 'BY_NC_SA': license = 'BY_SA'; break;
			}
			this.state.publishSetLicense(license);
			this.refresher.refreshStateAndDOM(this.state);
		};
		this.onLicenseCCND = (e: Event) => {
			...
			switch (this.state.current.ui.publishEditor.license) {
				case 'CC0':
				case 'BY': license = 'BY_ND'; break;
				case 'BY_SA': license = 'BY_ND'; break;
				case 'BY_NC': license = 'BY_NC_ND'; break;
				case 'BY_ND': license = 'BY'; break;
				case 'BY_NC_ND': license = 'BY_NC'; break;
				case 'BY_NC_SA': license = 'BY_NC_ND'; break;
			}
			this.state.publishSetLicense(license);
			this.refresher.refreshStateAndDOM(this.state);
		};
	...
	private getFormValues(): { title: string | null, desc: string | null } {
		const titleElem = document.getElementById('publish-title') as HTMLInputElement | null;
		const descElem = document.getElementById('publish-desc') as HTMLTextAreaElement | null;
