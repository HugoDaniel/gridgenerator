...
import { FillId, UIFillPath } from '../../../../data';
...
function buildPaths(selected: FillId, paths: Map<FillId, UIFillPath>, onAction: ((fillId: FillId) => void) | null, accum: any[]): any {
	let i: number = 0;
		...
		accum.push(
			<g key={`pathuse-${id}`}>
				<use className={'hover-orange-stroke pointer'}
					{...xlink} fill={path.fill} onClick={action} onTouchEnd={action} stroke-width={20} clip-path={`url(#clip-${id})`} />
			</g>
		);
	}
	return accum;
...
function buildShapes(props: IColorShapesProps) {
	...
	result.push(
		<path key={'template-path-shape'} d={props.templatePath} stroke={'#333333'} fill={'transparent'} stroke-width={10} />
	);
	return result;
}
