...
export class ColorCode extends Component<any, any> {
	constructor(props, context) {
		super(props, context);
		this.state = { r: props.color.r, g: props.color.g, b: props.color.b, hex: RGBColor.toHex(props.color) };
	}
	...
	public render() {
		...
		return (
			<div className={`ColorCode bg-near-white top-0 absolute ${props.className || ''}`} style={props.style}>
				<div className="flex flex-column items-start justify-center mt5">
					<div className="flex">
						<p className="sans-serif mh3">RGB</p>
						<input onInput={linkEvent(this, this.handleR)} value={this.state.r} defaultValue={`${props.color.r}`} className="f4 sans-serif w3 pa1" maxLength={3} placeholder="R"/>
						<input onInput={linkEvent(this, this.handleG)} value={this.state.g} defaultValue={`${props.color.g}`} className="f4 sans-serif w3 pa1" maxLength={3} placeholder="G"/>
						<input onInput={linkEvent(this, this.handleB)} value={this.state.b} defaultValue={`${props.color.b}`} className="f4 sans-serif w3 pa1" maxLength={3} placeholder="B"/>
					</div>
					<div className="flex mt2 items-center">
						<p className="sans-serif mh3">HEX</p>
						<input onInput={linkEvent(this, this.handleHex)} value={this.state.hex} defaultValue={RGBColor.toHex(props.color)} className="f4 sans-serif w4 pa1" maxLength={7} />
