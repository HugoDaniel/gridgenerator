	...
	for (let i = 0; i < props.shapes.length; i++) {
		result.push(
			<button
				...
				onClick={linkEvent({ d: props.shapes[i], index: i }, props.onFigureAction)}
			>
				<svg
	...
	if (!hasShapes) {
		return (
			<div
				...
				>
				<Button className="mr0-ns blue b--blue" bg="near-white" label="Change Shape Template" onAction={props.onEnterTemplateSelector} />
			</div>
		);
	} else {
		return (
