	...
	return (
		<div>
			...
			<div className="flex items-start justify-center">
				...
				<img className={imgcx} src={badge_earlyadopter} alt="early adopter" title="Early Adopter" /> : <div />
				}
				{ badges.indexOf('supporter') !== -1 ?
				<img className={imgcx} src={badge_standardaccount} alt="supporter" title="Supporter" /> : <div />
				}
			</div>
		</div>
	);
...
export const ProfileMe = (props: IProfileMeProps) =>
	<section
		className={`ProfileMe user-select ${props.className}`}
	>
		...
		<div className="pt4 pa5-ns pb0-ns flex flex-column items-start justify-center">
			...
			<TextArea className={inputcx + ' h4'} placeholder={props.profileAbout} name="profile-bio" id="profile-bio" disabled={props.isLoading} value={props.profileForm ? props.profileForm.about : null} />
			<div className="dt">
				<Button
					className="ml2 dtc"
				...
				/>
				{ props.isLoading ? <p className={neutralCx}>Updating</p>
				: <p className={props.hasError ? errorCx : successCx}>{props.loadingMsg}</p>
				}
			</div>
		</div>
	</section>
