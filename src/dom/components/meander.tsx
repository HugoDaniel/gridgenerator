		...
		if (props.meander.profile.id && props.meander.profile.name && props.meander.profile.created) {
			return(
				<ProfileSection {...defaultProps}
					...
					isLoading={props.meander.profile.loadingStatus === ProfileStatus.Loading}
...
export const MeanderFull = (props: IMeanderProps) => {
	const mainMenuProps: IMainMenuProps = {
		...
		className: `fixed bottom-0 left-0 w-100 transition-bg ${props.meander.course !== MeanderCourse.Project ? 'bt pt3 b--light-gray bg-white' : 'bg-transparent'}`
	};
	return (
		<div
			className={`MeanderFull fixed sans-serif transition-transform
				...
				${props.className || ''}`}
		>
			{props.meander.course === MeanderCourse.Project ? <div /> :
			<div
				className="children"
