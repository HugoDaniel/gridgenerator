...
export class FatState {
	...
	public static revive(o: FatStateReviver, state: State) {
		...
		result._mods = o.m.map((m) =>
			Modification.revive(m, FatState.reviveArgs));
		result._version = o.v;
		result._state = state;
		return result;
	}
	...
	public static reviveArgs(name: string, args: any[] | null): any[] | null {
		...
		switch (name) {
			case 'hudEnterNewFill':
				return [args[0], args[1], args[2].map(
					(c) => new RGBColor(c.r, c.g, c.b, c.a))];
			case 'shapeClose':
				return [new Vector2D(args[0].x, args[0].y), args[1].map(
					(c) => new RGBColor(c.r, c.g, c.b, c.a)), args[2]];
	...
	private mod(name: string, args: any[] | null) {
		...
		// create the mod
		this._mods.push(
			new Modification(this.version, Math.min(deltaT, this._maxDeltaT), name, args)
		);
	...
	public hudRotateShape(): FatState {
		...
		// update the shapes menu and fills menu
		this._state.ui.refreshMenus(
			layer, this._state.shapes, this._state.fills.buildShapeSVG(shape)
		);
