...
export function arcEllipse(cx: number, cy: number, h: number, v: number, a: number, pt: Vector2D): number {
...
export function tanEllipse(cx: number, cy: number, h: number, v: number, a: number, pt: Vector2D): LineObj {
	...
	let m2;
	if (isFinite(m1)) {
		m2 = (m1 * Math.cos(a) + Math.sin(a))
		   / (Math.cos(a) - m1 * Math.sin(a));
	} else {
...
export function plotEllipse(cx: number, cy: number, h: number, v: number, a: number, b: number): Vector2D {
...
export function isBetweenEllipseAngles(angle1: number, angle2: number, tangle: number): boolean {
...
export function intersectLinesSlope(m1: number, x1: number, b1: number, m2: number, x2: number, b2: number): Vector2D {
	const b = (b1 - b2);
