...
import { TextureAction, TextureChange, TextureChangeAlloc, TextureChangeUpdate } from './texture_actions';
...
export class TextureAtlas {
	...
	constructor(dpr: number, svgSize: number, glTextureSize: number, unitIndex: number, emptyAtlas: Uint8Array) {
		...
		this.changed = true;
		this.changes = [new TextureChangeAlloc(emptyAtlas, glTextureSize, glTextureSize)];
	}
	...
	private renderToArray(svg: string, img: HTMLImageElement, ctx: CanvasRenderingContext2D): Promise<Uint8Array> {
	...
	public addSvg(id: Vector2D, svg: string, img: HTMLImageElement, ctx: CanvasRenderingContext2D): Promise<TextureAtlas> {
		return new Promise((resolve, reject) => {
			...
			this.renderToArray(svg, img, ctx).then((arrData) => {
				...
				this.changed = true;
				this.changes.push(new TextureChangeUpdate(
					this.svgSize, this.textureCoords(index), arrData
				));
				resolve(this);
			}, reject);
		});
	...
	public updateSVG(id: Vector2D, svg: string, img: HTMLImageElement, ctx: CanvasRenderingContext2D): Promise<TextureAtlas> {
		return new Promise((resolve, reject) => {
			this.renderToArray(svg, img, ctx).then((arrData) => {
	...
	private gpuTextureAlloc(gl: WebGLRenderingContext, change: TextureChangeAlloc): Promise<WebGLTexture> {
		return new Promise( (resolve, reject) => {
	...
	public reset(emptyAtlas: Uint8Array) {
		...
		this.changed = true;
		this.changes.push(new TextureChangeUpdate(
			this.glTextureSize, new Vector2D(0, 0), emptyAtlas
		));
	}
}
