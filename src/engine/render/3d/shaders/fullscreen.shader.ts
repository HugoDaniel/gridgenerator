...
const enum FullscreenAnimations { None = 0, Open, Shift, Blur }
export const enum FullscreenMode { Full, Expand, Cols }
export class FullscreenShader {
	...
	constructor(canvas: WebGLContext, state: Readonly<State>, textures: TextureManager, clipspace: ClipSpace) {
		...
		this.openAnimation = new ShaderAnimation(duration, FullscreenAnimations.Open, ShaderAnimation.linearRev);
		this.openAmmount = 4;
...
*/
	...
	private _addColVertex(vData: number[], iData: number[], x1: number, x2: number, index: number): void {
		vData.push(x1); vData.push(-1.0); vData.push(0);
		vData.push(x2); vData.push(-1.0); vData.push(0);
		vData.push(x1); vData.push(1.0); vData.push(0);
		vData.push(x2); vData.push(1.0); vData.push(0);
		iData.push(index);
		iData.push(index + 1);
	...
	private _textureColData(tData: number[], w: number, cols: number, fstColPx: number) {
	...
	private initBuffersCols(gl: WebGLRenderingContext): ShaderBuffer[] {
		...
		const [shapeIds, delays, revDelays] = ShaderAnimation.initBuffers(gl, this._shapeIds(numberOfVerts), delaysArray);
	...
	private initShadersCols(gl: WebGLRenderingContext): ShaderProgram {
		const program = HelpersGL.initShaderProgram(gl, FullscreenShader.vShaderCols, FullscreenShader.fShaderCols);
